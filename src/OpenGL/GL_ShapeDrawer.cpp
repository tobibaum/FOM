...
void GL_ShapeDrawer::drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color,
		int	debugMode,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax,
		string textureFile, btScalar transparency)
{
	glEnable (GL_BLEND);
...
//		dz *= halfExtent[2];
	...
	if (shape->getShapeType() == UNIFORM_SCALING_SHAPE_PROXYTYPE)
	{
		...
		float	scalingFactor = (float)scalingShape->getUniformScalingFactor();
		{
			...
			drawOpenGL( (btScalar*)tmpScaling,convexShape,color,debugMode,worldBoundsMin,worldBoundsMax, textureFile,transparency);
		}
		glPopMatrix();
		return;
	}
	...
	if (shape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE)
	{
		...
		for (int i=compoundShape->getNumChildShapes()-1;i>=0;i--)
		{
			...
			drawOpenGL(childMat,colShape,color,debugMode,worldBoundsMin,worldBoundsMax, textureFile,transparency);
		}
...
//		drawCoordSystem();
		...
		if(textureFile!="")
		{
			...
			glBindTexture(GL_TEXTURE_2D,m_textureManager->getTextureHandle(textureFile));
		} else
		{
...
*/
			...
			case MULTI_SPHERE_SHAPE_PROXYTYPE:
			{
				...
				for (int i = multiSphereShape->getSphereCount()-1; i>=0;i--)
				{
					...
					drawOpenGL(childMat,&sc,color,debugMode,worldBoundsMin,worldBoundsMax, textureFile,transparency);
				}
...
//			btBvhTriangleMeshShape* trimesh = (btBvhTriangleMeshShape*) concaveMesh;
//
//			int numverts, stride, indexstride, subpart, numfaces;
//			unsigned char** vertexbase, indexbase;
//			PHY_ScalarType type,indicestype;
//
//			trimesh->getMeshInterface()->getLockedReadOnlyVertexIndexBase(vertexbase, numverts,type,stride,indexbase,indexstride,numfaces,indicestype,subpart);
...
//
void		GL_ShapeDrawer::drawShadow(btScalar* m,const btVector3& extrusion,const btCollisionShape* shape,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax, btScalar transparency)
{
	glPushMatrix(); 
//	for(int i=0;i<16;i++){
//		printf("%d: %f\n",i,m[i]);
			...
			if (shape->isConvex())
			{
				...
				for(int i=0;i<sc->m_edges.size();++i)
				{
					...
					if((d*btDot(sc->m_edges[i].n[1],extrusion))<0)
					{
						...
						glVertex3f(b[0],b[1]+extrusion[1],b[2]);
						glVertex3f(a[0],a[1]+extrusion[1],a[2]);
					}
...
//
GL_ShapeDrawer::GL_ShapeDrawer(TextureManager* textManager)
{
	m_texturehandle			=	0;
