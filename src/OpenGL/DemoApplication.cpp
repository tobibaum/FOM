...
m_cameraTargetPosition(0.f,17.f,0.f),
m_mouseOldX(0),
m_mouseOldY(0),
m_mouseButtons(0),
m_modifierKeys(0),
...
m_sundirection(btVector3(0,-2,1)*1000),
m_defaultContactProcessingThreshold(BT_LARGE_FLOAT)
{
	...
	m_shapeDrawer = new GL_ShapeDrawer (m_textureManager);
...
void DemoApplication::myinit(void)
{
	...
	GLfloat light_position0[] = { btScalar(0.0), btScalar(100.0), btScalar(0.0), btScalar(0.0 )};
...
//			glFrustum (-1.0, 1.0, -aspect, aspect, 1.0, 10000.0);
	...
	if(!m_cameraInitialized){
		Message msg;
		msg.messageType = Message::MSG_CAMERA_INITIALIZED;
		sendMessage(msg);
...
//		}
//	case 's' :
//		msg.messageType = Message::MSG_START_FLIGHT;
//		sendMessage(msg);
//
//		break;
//	case ' ':
...
void DemoApplication::learnDynamicsWorld(){
	m_dynamicsWorld = m_coloredWorld->getDynamicsWorld();
}
...
void	DemoApplication::renderscene(int pass)
{
	...
	list<Object*> objects = m_coloredWorld->getObjects();
	const int	numObjects = objects.size();
	list<Object*>::iterator obj_it = objects.begin();

	Object* current_obj;

	int shadows = 0;

//	printf("new renderloop\n ");
	for(int i=0;i<numObjects;i++)
	{
		current_obj = (*obj_it);

#ifndef RENDER_PHYSICAL_WALL
		if(current_obj->getType() == COL_WALL){
			// do not forget to count up object-iterator!
//			printf("wallObject in renderloop detected!!\n");
			obj_it++;
			((WallObject*)current_obj)->renderWall();
			continue;
		}
		else if (current_obj->getType() == COL_CAKE){
			obj_it++;
			continue;
		} else if (current_obj->getType() == COL_FINAL_CAKE){
			obj_it++;
			continue;
		} else if(current_obj->getType() == COL_BOUNDARY){
			obj_it++;
			continue;
		}
#endif
		...
		btCollisionObject*	colObj = current_obj->getCollObj();
//		btCollisionObject*	colObj = m_dynamicsWorld->getCollisionObjectArray()[j];
...
//		wireColor = btVector3(0.f,0.f,0.f);
		wireColor = current_obj->getColor();
...
//		m_dynamicsWorld->getDebugDrawer()->drawAabb(aabbMin,aabbMax,btVector3(1,1,1));
		...
		btScalar transparency = current_obj->getTransparency();
...
//		{
			...
			{
			case	0:
				if(current_obj->getType() != COL_PLAYER && current_obj->getType() != COL_SPARTA_POINTS){
					m_shapeDrawer->drawOpenGL(m,colObj->getCollisionShape(),wireColor,getDebugMode(),aabbMin,aabbMax, current_obj->getTexture(),transparency);
				}
				break;
			case	1:

				if(current_obj->getType() == COL_FLYING_OBJECTS){
					glMatrixMode(GL_MODELVIEW);
					glPushMatrix();
					glScalef(1,0,1);

					m_shapeDrawer->drawOpenGL(m,colObj->getCollisionShape(),wireColor,getDebugMode(),aabbMin,aabbMax, current_obj->getTexture(),.5);
					glPopMatrix();

				}
				break;
			case	2:
				if(current_obj->getType() == COL_SPARTA_POINTS){
					m_shapeDrawer->drawOpenGL(m,colObj->getCollisionShape(),wireColor,0,aabbMin,aabbMax,current_obj->getTexture(), transparency);
				}
				break;
			case 3:
				if(current_obj->getType() == COL_PLAYER ){
					glMatrixMode(GL_MODELVIEW);
					glPushMatrix();
					glScalef(1,0,1);
					btVector3 swap = wireColor;
					wireColor = btVector3(0,0,0);
					m_shapeDrawer->drawOpenGL(m,colObj->getCollisionShape(),wireColor,getDebugMode(),aabbMin,aabbMax, current_obj->getTexture(),0);
					glPopMatrix();
					wireColor = swap;
...
//
void DemoApplication::renderme(int pass)
{
	myinit();
